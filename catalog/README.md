...
- `catalog/sources/pl-products.json` — curated GS1 Poland (590) products. Manufacturer INCI when the brand publishes it (Ziaja). The app upserts this overlay on every catalog boot so packed CosIng/OBF sqlite does not need a full repack. Do not scrape retailers into this file.

The Kotlin pipeline (`CatalogBuilder`) validates that every `HIGH` / `PROHIBITED` ingredient has English and Polish comments, computes the SHA-256 catalog fingerprint, writes `catalog-manifest.json`, and packs `catalog/build/catalog.sqlite.gz`. `ShippedCatalogMerger` always includes the Polish overlay after fixtures (those GTINs win over ingest).

Online lookup for codes starting with `590` tries `pl.openbeautyfacts.org` / `pl.openfoodfacts.org` first and prefers `ingredients_text_pl` / `product_name_pl`.
