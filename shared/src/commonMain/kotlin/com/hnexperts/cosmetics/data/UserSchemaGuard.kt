...
object UserSchemaGuard {
    ...
    private fun addColumn(driver: SqlDriver, table: String, columnSql: String) {
        try {
            ...
            // Column already exists on databases created from the current schema.
        }
    }

    private fun backfillShelfKeys(driver: SqlDriver) {
        try {
            driver.execute(
                identifier = null,
                sql = SHELF_KEY_BACKFILL,
                parameters = 0
            )
        } catch (_: Exception) {
            // Old empty shelf tables may not have the new columns yet.
        }
    }
